<template>
  <div class="space-y-8 p-6 max-w-4xl mx-auto">
    <div class="text-center">
      <h1 class="text-3xl font-bold text-gray-900">FileUpload Component Examples</h1>
      <p class="text-gray-600 mt-2">Comprehensive examples with loading progress and upload features</p>
    </div>

    <!-- Original Form Example (Enhanced) -->
    <div class="p-6 rounded-lg border border-gray-200">
      <h2 class="text-xl font-semibold mb-4">Form Integration with Validation</h2>
      <div class="flex flex-col items-center gap-8 max-w-md mx-auto">
        <UForm :schema="schema" :state="state" class="space-y-4 w-full" @submit="onSubmit">
          <UFormField name="avatar" label="Avatar" description="JPG, PNG or WebP. 2MB Max." :size="size">
            <UFileUpload v-slot="{ open, removeFile }" v-model="state.avatar" accept="image/*" :show-progress="true"
              :upload-fn="realUpload" :auto-upload="false" @upload:success="onUploadSuccess"
              @upload:error="onUploadError">
              <div class="flex flex-wrap items-center gap-3">
                <UAvatar size="lg" :src="state.avatar ? createObjectUrl(state.avatar.file) : undefined"
                  icon="i-lucide-image" />
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <div v-if="state.avatar" class="text-xs space-y-1">
                    <p class="text-gray-600">{{ state.avatar.file.name }}</p>
                    ...
                    <UButton v-if="!state.avatar.uploading" label="Remove" color="error" variant="link" size="xs"
                      class="p-0" @click="removeFile()" />
                  </div>
                </div>
              </div>
            </UFileUpload>
    ...
    <div class="p-4 rounded-lg">
      <h3 class="font-semibold mb-3">Upload Simulation Settings</h3>
    ...
    <div class="p-6 rounded-lg border border-gray-200">
      <h2 class="text-xl font-semibold mb-4">Basic Upload with Progress</h2>
      <UFileUpload v-model="singleFile" label="Upload a file"
        description="Select a file to upload with progress tracking" :show-progress="true" :auto-upload="true"
        :upload-fn="simulateUpload" @upload:start="onUploadStart" @upload:progress="onUploadProgress"
        @upload:success="onUploadSuccess" @upload:error="onUploadError" @upload:cancel="onUploadCancel"
        class="w-full" />
    </div>

    <!-- Multiple Files with Auto Upload -->
    <div class="p-6 rounded-lg border border-gray-200">
      <h2 class="text-xl font-semibold mb-4">Multiple Files with Auto Upload</h2>
      <UFileUpload v-model="multipleFiles" multiple label="Upload multiple files"
        description="Files will be uploaded automatically when selected" :show-progress="true" auto-upload
        :upload-fn="simulateUpload" :allow-cancel="true" layout="list" @upload:start="onUploadStart"
        @upload:progress="onUploadProgress" @upload:success="onUploadSuccess" @upload:error="onUploadError"
        @upload:cancel="onUploadCancel" class="w-full" />
    </div>

    <!-- Grid Layout with Progress -->
    <div class="p-6 rounded-lg border border-gray-200">
      <h2 class="text-xl font-semibold mb-4">Grid Layout with Progress Overlay</h2>
      <UFileUpload v-model="gridFiles" multiple label="Upload images"
        description="Images will be displayed in a grid with progress overlay" accept="image/*" :show-progress="true"
        :auto-upload="true" :upload-fn="simulateUpload" layout="grid" @upload:start="onUploadStart"
        @upload:progress="onUploadProgress" @upload:success="onUploadSuccess" @upload:error="onUploadError"
        class="w-full" />
    </div>
    ...
    <div class="p-6 rounded-lg border border-gray-200">
      <h2 class="text-xl font-semibold mb-4">Advanced Progress Tracking</h2>
      <div class="space-y-4">
        ...
        <UFileUpload v-model="progressFiles" multiple label="Upload with detailed progress"
          description="Track overall and individual file progress" :show-progress="true" :auto-upload="false"
          :upload-fn="simulateUpload" layout="list" @upload:start="onUploadStart" @upload:progress="onUploadProgress"
          @upload:success="onUploadSuccess" @upload:error="onUploadError" @upload:cancel="onUploadCancel"
          class="w-full">
          <template #actions="{ files, uploadFile, removeFile, cancelUpload }">
            <div class="flex flex-wrap gap-2">
              <UButton v-if="files?.length" color="primary" @click="uploadAll(files, uploadFile)"
                :disabled="isUploading" icon="i-lucide-upload">
                Upload All ({{ files.length }})
              </UButton>
              <UButton v-if="files?.some(f => f.uploading)" color="warning" variant="outline"
                @click="cancelAllUploads(files, cancelUpload)" icon="i-lucide-x">
                Cancel All
              </UButton>
              <UButton v-if="files?.length" color="neutral" variant="outline" @click="removeAll(removeFile)"
                :disabled="files.some(f => f.uploading)" icon="i-lucide-trash">
                Clear All
              </UButton>
            </div>
          </template>
      ...
      <div class="p-6 rounded-lg border border-gray-200">
        <h2 class="text-xl font-semibold mb-4">Custom Upload Control</h2>
        <UFileUpload v-model="customFiles" multiple label="Manual upload control"
          description="Full control over upload process with custom actions" :show-progress="true" :auto-upload="false"
          :upload-fn="simulateUpload" layout="list" @upload:start="onUploadStart" @upload:progress="onUploadProgress"
          @upload:success="onUploadSuccess" @upload:error="onUploadError" @upload:cancel="onUploadCancel"
          class="w-full">
          <template #actions="{ files, uploadFile, removeFile, cancelUpload }">
          ...
          <template #file-progress="{ fileUpload }">
            <div class="flex items-center gap-2 w-full">
              <UProgress :value="fileUpload.progress || 0" :color="fileUpload.error ? 'error' : 'primary'" size="sm"
                class="flex-1" />
      ...
      <!-- Configuration Panel -->
      <div class="p-6 rounded-lg border border-gray-200">
        <h2 class="text-xl font-semibold mb-4">Component Configuration</h2>
      ...
      <div class="p-6 rounded-lg border border-gray-200">
        <h2 class="text-xl font-semibold mb-4">Original Examples with Progress</h2>
        <div class="space-y-6">
          <div>
            <h3 class="font-medium mb-2">Single File Upload</h3>
            <UFileUpload v-model="singleFile" :size="size" :variant="variant" :layout="layout" :position="position"
              label="Drop your image here" description="SVG, PNG, JPG or GIF (max. 2MB)" :show-progress="true"
              auto-upload :upload-fn="simulateUpload" :class="variant === 'area' ? 'w-full min-h-44' : ''"
              @upload:start="onUploadStart" @upload:progress="onUploadProgress" @upload:success="onUploadSuccess"
              @upload:error="onUploadError" />
          </div>

          <div>
            <h3 class="font-medium mb-2">Multiple Files with Custom Actions</h3>
            <UFileUpload v-model="multipleFiles" :size="size" :variant="variant" :layout="layout" :position="position"
              icon="i-lucide-image" label="Drop your images here" description="SVG, PNG, JPG or GIF (max. 2MB)" multiple
              :interactive="false" :show-progress="true" :upload-fn="simulateUpload" class="w-full min-h-44"
              @upload:start="onUploadStart" @upload:progress="onUploadProgress" @upload:success="onUploadSuccess"
              @upload:error="onUploadError">
              <template #actions="{ open }">
                <UButton label="Select images" icon="i-lucide-upload" color="neutral" variant="outline" :size="size"
                  @click="open()" />
              </template>
              ...
              <template v-if="layout === 'grid' || position === 'inside'" #files-top="{ open, files }">
                <div v-if="files?.length" class="mb-2 flex items-center justify-between">
                  ...
                  <UButton label="Add files" color="neutral" variant="outline" :size="size" class="-my-2"
                    @click="open()" />
                </div>
              </template>

              <template v-if="layout === 'list'" #files-bottom="{ removeFile, files }">
                <UButton v-if="files?.length" label="Remove files" color="neutral" variant="outline" class="self-start"
                  :size="size" @click="removeFile(0)" />
              </template>
            </UFileUpload>
          </div>
        </div>
      ...
      <div v-if="uploadEvents.length" class="p-6 rounded-lg border border-gray-200">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold">Upload Events Log</h2>
          <div class="flex gap-2">
            <UBadge :label="`${uploadEvents.length} events`" color="gray" variant="soft" />
        ...
        <div class="rounded-lg max-h-96 overflow-y-auto">
          <div v-for="(event, index) in uploadEvents" :key="index" class="p-3 border-b border-gray-200 last:border-b-0">
            <div class="flex items-start gap-3">
              ...
              <UBadge v-if="event.fileId" :label="event.fileId.slice(0, 8) + '...'" size="xs" color="gray"
                variant="soft" />
            </div>
          </div>
        </div>
      ...
      <div class="p-6 rounded-lg border border-gray-200">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <h3 class="font-semibold mb-3">New Props</h3>
            <div class="space-y-3 text-sm">
              <div class="p-3 rounded">
                <code class="font-mono text-xs">showProgress: boolean</code>
                <p class="text-gray-600 mt-1">Enable/disable progress display</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">allowCancel: boolean</code>
                <p class="text-gray-600 mt-1">Allow cancelling uploads in progress</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">uploadFn: Function</code>
                <p class="text-gray-600 mt-1">Custom upload function with progress callback</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">autoUpload: boolean</code>
                <p class="text-gray-600 mt-1">Automatically start upload when files are selected</p>
              </div>
            </div>
          </div>

          <div>
            <h3 class="font-semibold mb-3">New Events</h3>
            <div class="space-y-3 text-sm">
              <div class="p-3 rounded">
                <code class="font-mono text-xs">@upload:start</code>
                <p class="text-gray-600 mt-1">Fired when upload begins</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">@upload:progress</code>
                <p class="text-gray-600 mt-1">Fired during upload progress</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">@upload:success</code>
                <p class="text-gray-600 mt-1">Fired on successful upload</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">@upload:error</code>
                <p class="text-gray-600 mt-1">Fired on upload failure</p>
              </div>
              <div class="p-3 rounded">
                <code class="font-mono text-xs">@upload:cancel</code>
                <p class="text-gray-600 mt-1">Fired when upload is cancelled</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="space-y-8 p-6">
      <h1 class="text-2xl font-bold">FileUpload Component Examples</h1>

      <!-- Basic File Upload with Progress -->
      <div class="space-y-4">
        <h2 class="text-lg font-semibold">Basic Upload with Progress</h2>
        <UFileUpload v-model="singleFile" label="Upload a file"
          description="Select a file to upload with progress tracking" :show-progress="true" :auto-upload="false"
          :upload-fn="simulateUpload" @upload:start="onUploadStart" @upload:progress="onUploadProgress"
          @upload:success="onUploadSuccess" @upload:error="onUploadError" @upload:cancel="onUploadCancel" />
      </div>
      ...
      <!-- Multiple Files with Auto Upload -->
      <div class="space-y-4">
        <h2 class="text-lg font-semibold">Multiple Files with Auto Upload</h2>
        <UFileUpload v-model="multipleFiles" multiple label="Upload multiple files"
          description="Files will be uploaded automatically" :show-progress="true" :auto-upload="true"
          :upload-fn="simulateUpload" :allow-cancel="true" layout="list" @upload:start="onUploadStart"
          @upload:progress="onUploadProgress" @upload:success="onUploadSuccess" @upload:error="onUploadError"
      ...
      <!-- Grid Layout with Progress -->
      <div class="space-y-4">
        <h2 class="text-lg font-semibold">Grid Layout with Progress</h2>
        <UFileUpload v-model="gridFiles" multiple label="Upload images" description="Images will be displayed in a grid"
          accept="image/*" :show-progress="true" :auto-upload="true" :upload-fn="simulateUpload" layout="grid"
          @upload:start="onUploadStart" @upload:progress="onUploadProgress" @upload:success="onUploadSuccess"
          @upload:error="onUploadError" />
      </div>
      ...
      <div class="space-y-4">
        ...
        <UFileUpload v-model="customFiles" multiple label="Custom upload control"
          description="Manual upload control with custom actions" :show-progress="true" :auto-upload="false"
          :upload-fn="simulateUpload" layout="list">
          <template #actions="{ files, uploadFile, removeFile }">
            <UButton v-if="files?.length" color="primary" @click="uploadAll(files, uploadFile)" :disabled="isUploading">
              Upload All ({{ files.length }})
            </UButton>
            <UButton v-if="files?.length" color="neutral" variant="outline" @click="removeAll(removeFile)"
              :disabled="isUploading">
      ...
      <div v-if="uploadEvents.length" class="space-y-4">
        <h2 class="text-lg font-semibold">Upload Events Log</h2>
...
function onUploadStart(file: FileUploadFile, index: number) {
  addEvent('UPLOAD_START', `Started uploading: ${file.file.name}`, file.file.name)
}

function onUploadProgress(file: FileUploadFile, progress: number, index: number) {
  // Only log every 25% to avoid spam
  if (progress % 25 === 0 || progress === 100) {
    addEvent('UPLOAD_PROGRESS', `${file.file.name}: ${Math.round(progress)}%`, file.file.name)
  }
}

function onUploadSuccess(file: FileUploadFile, result: any, index: number) {
  addEvent('UPLOAD_SUCCESS', `Successfully uploaded: ${file.file.name} → ${result.url}`, file.file.name)
}

function onUploadError(file: FileUploadFile, error: string, index: number) {
  addEvent('UPLOAD_ERROR', `Failed to upload ${file.file.name}: ${error}`, file.file.name)
}

function onUploadCancel(file: FileUploadFile, index: number) {
  addEvent('UPLOAD_CANCEL', `Cancelled upload: ${file.file.name}`, file.file.name)
}
...
function getEventClass(type: string): string {
  ...
  return `mx-2 px-2 py-1 rounded text-xs ${classes[type] || 'text-gray-600'}`
}
...
async function onSubmit(event: FormSubmitEvent<schema>) {
  ...
  try {
    ...
    addEvent('FORM_ERROR', `Form submission failed: ${error.message}`)
  }
}
</script>
