...
Nuxt UI v4 marks a major milestone: **Nuxt UI and Nuxt UI Pro are now unified into a single, fully open-source and free library**. You now have access to 100+ production-ready components, all available in the `@eslamdevui/ui` package.
...
1. Replace `@eslamdevui/ui-pro` with `@eslamdevui/ui` in your `package.json`:

::code-group{sync="pm"}

```bash [pnpm]
pnpm remove @eslamdevui/ui-pro
pnpm add @eslamdevui/ui
```

```bash [yarn]
yarn remove @eslamdevui/ui-pro
yarn add @eslamdevui/ui
```

```bash [npm]
npm uninstall @eslamdevui/ui-pro
npm install @eslamdevui/ui
```

```bash [bun]
bun remove @eslamdevui/ui-pro
...
2. Replace `@eslamdevui/ui-pro` with `@eslamdevui/ui` in your `nuxt.config.ts`:
...
2. Replace `@eslamdevui/ui-pro` with `@eslamdevui/ui` in your `vite.config.ts`:

```diff [vite.config.ts]
import { defineConfig } from 'vite'
import vue from '@vitejs/plugin-vue'
- import uiPro from '@eslamdevui/ui-pro/vite'
...
4. Replace `@eslamdevui/ui-pro` with `@eslamdevui/ui` in your CSS:
...
5. Replace `@eslamdevui/ui-pro` with `@eslamdevui/ui` in your imports:

```diff
- import type { BannerProps } from '@eslamdevui/ui-pro'
...
```diff
- import { findPageHeadline } from '@eslamdevui/ui-pro/utils/content'
+ import { findPageHeadline } from '@nuxt/content/utils'

- import { findPageBreadcrumb } from '@eslamdevui/ui-pro/utils/content'
